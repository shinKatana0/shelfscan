...
> **`doc/guide.md` の翻訳。STALE — 英語版は `e99ffcf` より先へ
> 進んでおり、最後に動かしたのは `05145bd` です (2026-08-24)。**
> この翻訳は `85cbd54` 時点のもので、
...
- **`At least N spines could not be read`** — 一件の報告が複数の背表紙を指しうる
  ...
  写真のグループに入れます。
- **`Keyless run -- nothing was looked up`** — 書かれているとおりの意味で、
  どのカタログにも一度も問い合わせなかったということです。だからどの行も
  読み取られたままのタイトルです。「IGDB の段階が無かった」とは同じでは
  ありません。TMDB の資格情報があり IGDB の対が無い実行は、映画の行を照会
  するのでこのバナーは出ず、そのゲームの行が行ごとの印を持ちます。意図的に
  エラー色にしていません。あなたが選んだモードであり、その行を運べる
  書き出し先をバナー自体が名指します。
