...
/// The check that runs without this application (T-0354, T-0355), composed
/// from `unreachable.dart` since T-0357 rather than copied.
///
...
///
/// `the lookup` where the vision family says `the scan`: this is the resolve
/// stage, and `resolve` is also a command of its own that scans nothing. The
/// shared `any answer at all` is right here because both of these are POST
/// endpoints -- the search and the token request -- so a browser reaching
/// either gets an error page rather than anything friendly, and an error page
/// is the host answering.
final _outwardCheck =
