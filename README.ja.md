...
- **端から端まで検証できている経路は一つだけ。** 写真 → `.xcoll` → Tonkatsu Box
  ...
  **ディスク系入力源から出た `.xcoll` をカタログアプリに取り込んだことはない**
  ...
  である。
- **映画は照会される。ただしそのカタログに手が届くのはコマンドライン版だけで
  ある。** リリース名の形をした動画ファイルは、ゲームの行ではなく映画の行に
  なり、その種別はレビュー画面に表示され、そこで直せる。映画の行は TMDB へ
  送られ、どの設定でもゲームのカタログには入らない。しかしその照会を賄う資格
  情報はコマンドライン版の環境変数であり、**アプリにはそれを置く場所が無い**。
  だからアプリでは映画の行はどの経路でも鍵なしのままであり、その資格情報を
  持たないコマンドライン実行でも同じである。実在のリリース名に TMDB がどれ
  だけうまく答えるかは未計測である。経路を通しはしたが、測ってはいない
  （[どこまで来ているか](#films-are-read-as-films-and-how-far-that-goes)）。
...
**資格情報はコマンドライン版の環境変数である。`SHELFSCAN_TMDB_TOKEN` といい、
`.env.example` に載っている。アプリはそれをそもそも持てない。** 資格情報を OS
のキーチェーンに預けており、設定画面の欄は三つではなく二つだからである。
だからアプリでは、映画の行はどの経路でも鍵なしである。
