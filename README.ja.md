...
- **映画は読むが、映画側はレビューで止まっている。** リリース名の形をした
  ...
  どちらも TMDB を呼ばないので、映画は何とも一致せず、CSV には出るが `.xcoll`
  には決して出ない
...
どれだけうまく答えるかは「未計測」であって、「計測して悪かった」ではない。だから
映画の行は、ファイル名から読んだタイトルを持ってレビューに届き、何とも一致
せず、CSV には書き出されるが `.xcoll` には書き出されない。`.xcoll` は
カタログ ID のファイルであり、入れるものが無いからである——必要なカタログに
手が届かないときの、どの行とも同じふるまいである。
