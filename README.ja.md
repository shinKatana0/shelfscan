...
- **品質は与えたビジョンモデルの品質そのままであり、無料のものには既知の上限が <!-- measured-on: qwen2.5vl:7b -->
  ...
  *読める*。低解像度では日本語の背表紙を落とすが、そこではそれが正しい答え
  ...
  だけで、それには金がかかる（[実測した差](#which-model-and-what-it-changes)）。 <!-- /measured-on -->
...
| 映画とアニメの ID（およびそれに伴う `.xcoll` の行） | **$0**。ただし自分で登録する無料の TMDB アカウントが要る（[経路 B](#path-b--bring-your-own-keys)）。 |
...
*あなたが*設定したビジョンモデルへ、それ以外へは送らない——と、解決段階が
カタログへ送るタイトル文字列だけである。ゲームの行は IGDB へ、映画とアニメの行
は TMDB へ送られ、どちらのカタログにも画像は決して送られない。ほかにマシンを
出るのは、それぞれのカタログが要求する資格情報だけである——あなたの Twitch の
client id と secret はアクセストークンのために `id.twitch.tv` へ、あなたの TMDB
のトークンは検索のたびに TMDB へ行く——そして資格情報を持っていないカタログには
一切接続しない。何がどこへ行くかは
...
**これは `0.x` のビルドである。** これは謙遜ではなく意図である。このプログラムが
書き出す二つのファイル形式はまだ形が変わりうるので、あるバージョンが書いた
レビュー文書が次のバージョンで読めることは約束していない。その理由は
...
| IGDB の ID（ゲームの行） | なし | あり |
| TMDB の ID（映画とアニメの行） | なし | あり |
| CSV 書き出し | 可 | 可 |
| `.xcoll` 書き出し | 不可（カタログの ID が要る） | 可 |
| 登録 | 不要 | IGDB には Twitch アプリケーション、映画とアニメには TMDB アカウント——どちらも無料で、片方だけでもその行には足りる |
...
- **`--fallback` — 上のどれかに重ねる二つ目のモデル:** これは一つ目のモデルが
  ...
  述べられる。
- **IGDB（解決段階、ゲームの行）:** ビジョンモデルが読んだタイトル文字列を受け
  取るのであって、画像は決して受け取らない。またあなた自身の Twitch の
  client id と secret がアクセストークンのために `id.twitch.tv` へ行く。この
  資格情報がなければゲームの行は照合されないまま残り、どちらのサービスにも一切
  接続しない。
- **TMDB（解決段階、映画とアニメの行）:** それらの行が読まれたタイトル文字列
  を——ファイル名が公開年を含んでいればその年も——受け取るのであって、画像は
  決して受け取らない。またあなた自身の API Read Access Token が検索のたびに
  TMDB へ行く。これがなければそれらの行は照合されないまま残り、TMDB には一切
  接続しない。
...
走らせ方と、変更が黙って壊してはいけないものが書いてある。このリポジトリが
何を公開し、何を公開しないか
