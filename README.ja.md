...
ゲームではなく映画として読まれる。ただしその半分は
[まだ仕上がっていない](#films-are-read-as-films-and-how-far-that-goes)。
一度の実行で得たものは
...
- **二つのプラットフォームのうち Android のほうが手薄である。** Windows も
  Android もこのリポジトリからビルドして動かしているが、構造上 Windows 限定の
  ものが二つある。スマートフォンの HEIC 写真の変換は Windows Imaging Component
  を使い、GOG Galaxy ライブラリは Galaxy 自身のデータベースから読むが、Galaxy は
  Windows 用プログラムである。インストーラも配布バイナリもなく、ソースから
  ビルドする（[セットアップ](#setup)）。
...
- **端から端まで検証できている経路は一つだけ。** 写真 → `.xcoll` → Tonkatsu Box
  ...
  **ディスク系入力源から出た `.xcoll` をカタログアプリに取り込んだことはない**
  ...
  である。
- **映画は読むが、映画側はレビューで止まっている。** リリース名の形をした
  動画ファイルは、ゲームの行ではなく映画の行になり、その種別はレビュー画面に
  表示され、そこで直せる。繋がっていないのは照会のほうである。二つのシェルの
  どちらも TMDB を呼ばないので、IGDB の資格情報が無ければ映画は何とも一致せず、
  有ればゲームのカタログで検索され、ゲームに一致して返ってくることがある。
  `.xcoll` はその行を拒否して名前を挙げるが、CSV はそのまま書き出し、行のどこ
  にも映画であることを示すものは無い
...
**まだ無いものを、そのまま述べる。そうしないと上の段落が約束として読まれる。**
映画は IGDB ではなく TMDB で照会されるはずであり、それを行うクライアントは
書かれていてテストもある——ただし偽のサーバに対してである。**今日、TMDB を
呼ぶ実行は一つもない。** 二つのシェルのどちらもそのクライアントを繋いでおらず、
TMDB の資格情報をここで使ったこともないので、実在のリリース名に TMDB が
どれだけうまく答えるかは「未計測」であって、「計測して悪かった」ではない。

**そのあと映画の行がどうなるかは、あなたが既に持っている資格情報しだいであり、
気をつけるべきは二つめの場合である。** IGDB の資格情報が無ければ、行はファイル名
から読んだタイトルを持ってレビューに届き、何とも一致せず、CSV には書き出される
が `.xcoll` には書き出されない。`.xcoll` はカタログ ID のファイルであり、入れる
ものが無いからである——必要なカタログに手が届かないときの、どの行とも同じ
ふるまいである。**IGDB の資格情報が有れば、その行はゲームのカタログで検索
される。** シェルが作るリゾルバは一つだけで、それは IGDB のものであり、どの種別の
行を渡されたのかは誰も伝えないからである。題名がゲームと同じ映画——映像化作品は
ほぼ定義からして題名を共有する——は、そのゲームの正式名称・プラットフォーム・
確信度を持って返ってくることがあり、うまくいった行のように見える。

**それを最後に捕まえられるのは、二つの書き出しのうち一方だけである。** `.xcoll`
は、種別が含意するカタログから来た ID であることを求めるので、その行を拒否し、
書き出しが落とした行として名前を挙げる。CSV には種別の列がそもそも無く——そこの
`media_type` は物理的な媒体、`cartridge`/`disc`/`unknown` である——だから行は
ゲームの題名・ゲームのプラットフォーム・ゲームの ID を持って出ていき、映画で
あることを示すものはどこにも無い。*全行を確認する*のはまさにこのためであり、
題名の隣に出ている種別だけがそれを見破らせる。詳しい説明は
...
**アニメは動いている種別ではない。** レビュー画面はその値を出し、人が行を
それに直すこともできるが、それを生む入力源は無く、答えるカタログも無く、
`.xcoll` はその行を正面から拒否する。形式がその隣に要求する「映画か
シリーズか」の印を、でっち上げないためである。
...
鍵なしの実行では全行がそれにあたる。今日何かを書く接頭辞は `igdb:` だけである。
二つ目を持ちうるのは映画の行だが、そのカタログを呼ぶ実行はまだ一つもない
（[映画](#films-are-read-as-films-and-how-far-that-goes)）。
